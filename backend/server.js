...
// votes - absentee
app.get('/:state/absentee-mail-votes', (req, res) => {
  console.log('absentee votes endpoint hit');
...
// votes - total
// change to sum off other methods
...
app.get('/:state/:candidate/party', (req, res) => {
  ...
  const searchTerm = rawCandidate
    .split('-')
    .join(' ')
    .toLowerCase();

  db.all(
    `SELECT name, party FROM nc_candidates WHERE LOWER(name) LIKE ?`,
    [`%${searchTerm}%`],
    (err, rows) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(rows);
